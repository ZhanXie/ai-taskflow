/**
 * Tencent Hunyuan API 测试工具
 * 使用此脚本测试API连接和认证
 * 
 * 使用方法（在Node.js环境）：
 * node -r ts-node/register app/lib/test-hunyuan.ts
 * 或编译后运行
 */
  ...
  try {
    ...
    const payload = {
      ...
      Messages: [
        {
          Role: "user",
          Content: "Hello, please respond with a JSON object containing {\"test\": true}",
        },
      ],
    };
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        console.log(`   内容：${JSON.stringify(parsed).substring(0, 100)}...\n`);
      } else {
